...
def do_when(conditional, command, *args, **kwargs):
    ...
    sublime.set_timeout(
        functools.partial(do_when, conditional, command, *args, **kwargs), 50
    )
            ...
            if e.errno == 2:
                ...
                if not _has_warned:
                    ...
                    output = "{cmd} binary could not be found in PATH\n\nConsider using the {cmd_setting}_command setting for the Git plugin\n\nPATH is: {path}".format(
                        cmd=self.command[0],
                        cmd_setting=self.command[0].replace("-", "_"),
...
class GitCommand(object):
    ...
    def run_command(
        ...
        **kwargs,
        ...
        if (
            ...
            kwargs[str("fallback_encoding")] = str(
                self.active_view()
                .settings()
    ...
    def generic_done(self, result, **kw):
        if (
            ...
            if self.active_view().is_dirty():
                ...
                do_when(
                    lambda: not self.active_view().is_loading(),
    ...
    def _output_to_view(
        ...
        **kwargs,
        ...
        args = {"output": output, "clear": clear}
...
class GitWindowCommand(GitCommand, sublime_plugin.WindowCommand):
    ...
    def fallback_encoding(self):
        if self.active_view() and self.active_view().settings().get(
            "fallback_encoding"
        ):
            return (
                self.active_view()
